...
use neutrino_store::{AncestryGap, StagedKind, StagedPdu, StagingStore, StorageError};
...
impl StagingStore for SqliteStore {
    async fn stage_pdu(
        ...
        let live = matches!(kind, StagedKind::Live);
        ...
        self.run_write(move |conn| -> Result<bool, Error> {
            ...
            let inserted = conn.execute(
                "INSERT OR IGNORE INTO staged_events (event_id, room_id, origin, live, json) \
                 SELECT ?1, ?2, ?3, ?4, ?5 \
                 WHERE NOT EXISTS (SELECT 1 FROM events WHERE event_id = ?1)",
                params![event_id, room_id, origin, live, json],
            )?;
            Ok(inserted == 1)
        })
        .await
    ...
    async fn staged_for_room(&self, room_id: &RoomId) -> Result<Vec<StagedPdu>, StorageError> {
        ...
        self.run_read(move |conn| -> Result<Vec<StagedPdu>, Error> {
            let mut stmt = conn.prepare(
                "SELECT event_id, origin, live, json FROM staged_events WHERE room_id = ?",
            )?;
            ...
            let mut out = Vec::new();
            for r in rows {
                let (event_id, origin, live, json) = r?;
...
#[cfg(test)]
mod tests {
    use neutrino_store::{EventStore, StagedKind, StagedPdu, StagingStore};
    ...
    #[tokio::test]
    async fn stage_pdu_records_kind_and_keeps_first_on_duplicate() {
        // `kind` round-trips per row (the worker's timeline gap-fill trigger
        // keys on it), and — like `origin` — the first stage of an id wins.
        let (s, create) = store_with_room_and_create().await;
        let x = state_event(&[create.event_id.as_ref()], 1);
        let y = state_event(&[create.event_id.as_ref()], 2);
        ...
        let kind_of = |rows: &[StagedPdu], id: &OwnedEventId| {
            rows.iter()
                .find(|p| &p.event_id == id)
                .map(|p| p.kind)
                .expect("row present")
        ...
        let rows = s.staged_for_room(*ALICE_ROOM_ID).await.unwrap();
        assert_eq!(kind_of(&rows, &x.event_id), StagedKind::Live);
        assert_eq!(kind_of(&rows, &y.event_id), StagedKind::Fetched);

        // Re-staging X under a different kind is an INSERT OR IGNORE no-op.
        s.stage_pdu(
            origin(),
            *ALICE_ROOM_ID,
            &x.event_id,
        ...
        let rows = s.staged_for_room(*ALICE_ROOM_ID).await.unwrap();
        assert_eq!(
            kind_of(&rows, &x.event_id),
            StagedKind::Live,
            "first kind must win on a duplicate stage"
        );
    }
