...
-- restarts. The federation `server_name` is the iroh node id derived from this
-- secret (its ed25519 public key), so the identity MUST survive restarts —
-- hence persisting the secret, not just a session key. Single row, pinned to
-- `id = 0`. No `user_version` bump (additive, no live data, no migration
-- framework — same policy as the other late tables).
