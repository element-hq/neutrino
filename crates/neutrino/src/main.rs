...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = neutrino_main::Config::from_env();
    let store = neutrino_main::open_store(&config).await?;
    neutrino_main::entrypoint(config, store, commands_rx, None, None, None, None).await
}
