...
### Concurrent same-conn requests cancel the older, they don't reject the newer
MSC3575/4186 say concurrent requests with the same `conn_id` are illegal but don't pin the server's response. We cancel the in-flight long-poll on the older request and let the newer one proceed; the cancelled request returns its last `build_response` output (typically empty) with `conn.pos` advanced and the response cached for idempotency. Implementation in `ConnEntry::cancel_gen` (a `watch::Sender<u64>`); see `mod.rs::handle` for the bump-before-lock ordering. Body-differing newer requests at the same `pos` will see the advanced `conn.pos` after the cancel completes and return `M_UNKNOWN_POS` rather than processing fresh against the old pos — accept one re-init round-trip in that case. Byte-identical newer requests hit the freshly-written idempotency cache, no re-init needed.
