...
There is **one bridge network per pair** of servers, named after the two server
numbers — `net_12`, `net_13`, `net_23`. The only network path between two servers
is their shared pair-network,
...
./nctl partition cut  13                  # isolate hs1 <-> hs3 (both ways)
