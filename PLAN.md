...
2026-05-19: Phase 1 split into 1a (pure-JSON wire format, complete) and 1b (referential validation, blocked on Phase 5 provider). 1b is `validate::validate_references(event, provider)` and owns: v12 rule 2 (room_id corresponds to an accepted create event) and the MSC4242 `prev_state_events` triad (same-room, has-state-key, not-rejected). Rule 2 moves out of Phase 3 — it's existential, not authorisation. Phase 3 keeps rule 4+ (rules that make decisions on content, not just existence). `apply` orchestration calls 1a → 1b → state-res → 3, never implements validation itself.
